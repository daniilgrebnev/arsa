...
export const DriverTree = () => {
  ...
  const { data, filteredData } = useSelector((state: RootState) => state.driver)
  const getTree = (data) => {
  ...
  const groups = typeof data !== "string" && data != null ? driversCheckboxTree(filteredData) : []
  console.log(uniq(groups))
  return (
    <>
