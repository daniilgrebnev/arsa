...
export const Calendar: React.FC<propsType> = ({
  ...
  const visibleDays = useMemo(() => {
    ...
    return [
      ...[...prevMonthArr].slice(
        prevMonthArr.length - countPrevDay.needDisplayedCount
      ),
      ...currentMonthArr,
  ...
  // при изменении года или месяца меняем currentDate
  const getYears = () => {
    let years: Array<number> = [];
    for (let year = 2015; year <= currentDate.year; year++) {
      years.push(year);
    }
    return years;
  };
  // активный день храним отдельно в глобальном сторе
    ...
    <>
      <div className="calendar">
        ...
        {/* <div className="select-date">
          <div className="select-date__year">
            {getYears().map((year, key) => (
              <div key={key}>{year}</div>
            ))}
          </div>
          <div className="select-date__months">
